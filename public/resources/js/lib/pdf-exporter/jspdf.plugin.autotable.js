...
function createModels(inputHeaders, inputData) {
    ...
    inputData.forEach(function (rawRow, i) {
        ...
        table.columns.forEach(function (column) {
        	var data;
        	var association = column.dataKey.split('\.');
        	
        	for (var index in association) {
        		if(index == 0) {
        			data = rawRow[association[index]];
        		} else {
        			data = data[association[index]];
        		}
        	}
        	
            var cell = new models_1.Cell(data);
